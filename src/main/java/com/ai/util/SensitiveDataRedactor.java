...
public final class SensitiveDataRedactor {
    private static final Set<String> SENSITIVE_FIELD_NAMES =
            Set.of("password", "token", "secret", "apikey", "authorization", "cookie");
        ...
        for (String field : SENSITIVE_FIELD_NAMES) {
            ...
            Pattern colonPattern =
                    Pattern.compile(
                            "([\"']?" + field + "[\"']?\\s*:\\s*[\"']?)([^\"',}\\n]+)",
                            Pattern.CASE_INSENSITIVE);
            redacted = colonPattern.matcher(redacted).replaceAll("$1" + REDACTED);
            ...
            Pattern equalsPattern =
                    Pattern.compile(
                            "([\"']?" + field + "[\"']?\\s*=\\s*[\"']?)([^\"',}\\s]+)",
                            Pattern.CASE_INSENSITIVE);
            redacted = equalsPattern.matcher(redacted).replaceAll("$1" + REDACTED);
        }
