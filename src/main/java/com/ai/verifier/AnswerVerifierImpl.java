...
public class AnswerVerifierImpl implements AnswerVerifier {
    ...
    public VerificationResult verify(Answer answer, RetrievalResult context) {
        ...
        logger.info("Starting answer verification - answerLength: {}, contextChunks: {}",
            answer.text().length(), context.chunks().size());
        ...
        for (Claim claim : claims) {
            ...
            String supportingChunkId = isGrounded && !context.chunks().isEmpty()
                ? context.chunks().get(0).id()
                : null;
        ...
        String reasoning = String.format("Verified %d/%d claims as grounded (%.2f%% grounding)",
            groundedCount, claims.size(), groundingScore * 100);

        logger.info("Verification completed - status: {}, groundingScore: {:.2f}",
            status, groundingScore);
    ...
    private List<Claim> extractClaims(String answerText) {
        ...
        try {
            String response = ollamaClient.generate(VERIFICATION_MODEL, prompt, MAX_VERIFICATION_TOKENS);
            return parseClaimsFromResponse(response);
        } catch (Exception e) {
            logger.warn("Failed to extract claims via LLM, treating answer as single claim: {}",
                e.getMessage());
    ...
    private List<Claim> parseClaimsFromResponse(String response) {
        ...
        return claims.isEmpty()
            ? List.of(new Claim(response.trim(), false, null))
            : claims;
    }
