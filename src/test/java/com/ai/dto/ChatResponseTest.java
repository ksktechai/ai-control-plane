...
class ChatResponseTest {
    ...
    void shouldRejectNullAnswer() {
        assertThatThrownBy(() -> new ChatResponse(
    ...
    void shouldRejectNegativeConfidence() {
        assertThatThrownBy(() -> new ChatResponse(
            "answer", List.of(), -0.1, "llama3.1:8b", "SIMPLE", "GROUNDED"
        )).isInstanceOf(IllegalArgumentException.class)
    ...
    void shouldRejectConfidenceAboveOne() {
        assertThatThrownBy(() -> new ChatResponse(
            "answer", List.of(), 1.1, "llama3.1:8b", "SIMPLE", "GROUNDED"
        )).isInstanceOf(IllegalArgumentException.class)
          .hasMessageContaining("Confidence must be between 0.0 and 1.0");
    }
    ...
    void shouldReturnImmutableCitations() {
        ...
        ChatResponse response = new ChatResponse(
            "answer", citations, 0.9, "llama3.1:8b", "SIMPLE", "GROUNDED"
        );

        assertThatThrownBy(() -> response.citations().add(
    ...
    void shouldImplementEqualsCorrectly() {
        ...
        ChatResponse r3 = new ChatResponse(
            "different", citations, 0.9, "llama3.1:8b", "SIMPLE", "GROUNDED"
        );
        ChatResponse r4 = new ChatResponse(
            "answer", differentCitations, 0.9, "llama3.1:8b", "SIMPLE", "GROUNDED"
        );
        ChatResponse r5 = new ChatResponse(
            "answer", citations, 0.8, "llama3.1:8b", "SIMPLE", "GROUNDED"
        );
        ChatResponse r6 = new ChatResponse(
            "answer", citations, 0.9, "different-model", "SIMPLE", "GROUNDED"
        );
        ChatResponse r7 = new ChatResponse(
            "answer", citations, 0.9, "llama3.1:8b", "DEEP", "GROUNDED"
        );
        ChatResponse r8 = new ChatResponse(
            "answer", citations, 0.9, "llama3.1:8b", "SIMPLE", "UNGROUNDED"
        );
